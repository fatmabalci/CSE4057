            ...
            try:
                ...
                for value in read:
                    ...
                    if value is self.tcp_server_socket:
                        ...
                        print(self.connected_user_name, message_splitted)
                        ...
                        elif message_splitted[0] == "NONCE":
                            print(self.connected_user_name)
                            user_private_key_value = RSA.import_key(open(f"{self.connected_user_name}_private.pem").read())
                            value = decrypt_text(message_splitted[1], user_private_key_value)
                            
                            if str(nonce_value) == str(value):
                                print("Chat is verified with the ACK")
                            else:
                                self.chat_status = 0
                        # Check if the message is OK
                        elif message == "OK":
                            nonce_value = random.randint(3, 9)

                            nonce_message = "NONCE" + "\t" + str(nonce_value)

                            value.send(nonce_message.encode())
...
class UserClient(threading.Thread):
    ...
    def run(self):
        ...
        if self.user_server.user.status == 1 and self.client_response is None:
            # prepare the command
            request_message = "CHAT-REQUEST\t" + str(self.user_server.user.port) + "\t" + self.user_server.user.user_name + "\t" + "Hello"
        ...
        # check if the message was OK
            ...
            nonce = self.tcp_client_socket.recv(HEADER).decode()
            # save the log
            save_info_log("Received from " + self.ip + ":" +
                         str(self.port) + " -> " + self.client_response)
            nonce_message = nonce.split("\t")
            print("Nonce is " + nonce_message[1]) 
            print(self.user_server.user.user_name)
            user_public_key_value = RSA.import_key(open(f"{self.user_server.user.user_name}_public.txt").read())
            encrypt_text("encrypted_nonce.bin", user_public_key_value, str(nonce_message[1]).encode('utf-8'))

            nonce_encrypted_message = "NONCE" + "\t" + "encrypted_nonce.bin"
            self.tcp_client_socket.send(nonce_encrypted_message.encode())
            
            ack = self.tcp_client_socket.recv(HEADER).decode()
...
class ClientOperationHandler:
    ...
    def register(self, username, password):
        ...
        certificate = bytes.fromhex(response)

        result = verify(username, certificate)
        if result:
            save_certificate(username, response)
        print(f"Certificate verification {result}")
