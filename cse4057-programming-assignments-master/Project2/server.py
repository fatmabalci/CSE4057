...
class ServerBase(threading.Thread):
    ...
    def __init__(self):
        ...
        logging.info("Alper and Ömer says hello :)")
        print("Program is starting...")
        print("Alper and Ömer says hello :)")
        ...
        while True:
            try:
                ...
                if message[0] == "JOIN":
                    ...
                    certificate = sign(message[1])

                    user_certificates[message[1]] = certificate

                    print("From " + self.ip + ":" +
                          str(self.port) + " " + response)
                    logging.info("Message sent " + self.ip + ":" +
                                 str(self.port) + " " + certificate.hex())
                    self.tcp_client_socket.send(certificate.hex().encode())
